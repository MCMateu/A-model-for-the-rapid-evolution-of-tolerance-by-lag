...
int main (void)
{
	...
	T = gsl_rng_default;
      	r = gsl_rng_alloc (T);
	gsl_rng_set(r,3731298); //cambiar la semilla, pero despues de llamar al generador.
			...
			u1=SPmax*(gsl_rng_uniform (r));
				...
				else if(SP[0]<u1 && u1<=SP[1])
				{
					...
					{					
						...
						w=gsl_rng_uniform_int (r,Gtot)+1;
						Gtot=Gtot+1;
						ZG[Gtot]=1.0/(1.0/ZG[w]+gsl_ran_gaussian_tail(r, -(1.0/ZG[w]), mutaciones));
						ZG[w]=1.0/(1.0/ZG[w]+gsl_ran_gaussian_tail(r, -(1.0/ZG[w]), mutaciones));	
						Ntot=Gtot+Ztot;
					}
					else
					{
						...
						ZG[w2]=1.0/(1.0/ZG[w]+gsl_ran_gaussian_tail(r, -(1.0/ZG[w]), mutaciones));
