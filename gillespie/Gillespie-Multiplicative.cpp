...
int main (void)
{
	...
	double suma,s,t1,t2,scale,sum,period,mutaciones,media,varianza;
	...
	int time_frames;
					//RATE
				/*****/ z=1.0;						
				/*****/	d=3.6E-5; //DEATH BY NATURAL CAUSES
				/*****/	b=2.40; // BIRTH 
				/*****/	s=0.12; //ENTER DORMANT STATE
				/*****/	mutaciones=0.048; //AMPLITUDE OF THE MUTATION	
					/*****/
					K=100000; //MAXIMUM CARRYING CAPACITY

				//SIMULATION PARAMETERS
				period=23.0; //DURATION OF A WHOLE CYCLE
				sigma=1.27; //STANDARD DEVIATION OF THE INITIAL TRUNCATED GAUSSIAN
				mu=0.0; //MEAN OF THE INITIAL TRUNCATED GAUSSIAN
				ciclos=1200; //TOTAL NUMBER OF CYCLES
	...
	T = gsl_rng_default;
	r = gsl_rng_alloc (T);
	gsl_rng_set(r,84565667); //SEED OF THE GENERATOR
	...
	//FILES
				...
				f1=fopen("time-tau-P-3h-3030.txt","w");
			
		

				interval=0.5; //INTERVAL OF TIME BETWEEN MEASURES INSIDE A CYCLE
				
				step=0.1; //BIN SIZE OF THE HISTOGRAM

				max=60.0; //SIZE OF THE PHENOTYPIC SPACE (STARTING FROM 0)
	
				seed=1; //TOTAL NUMBER OF TEMPORAL SERIES
					
				time_frames=int(23/interval)+2; //add +1 to avoid the thero index of c++  vectors (in the vector declaration), +1 to include the t=0
							     //and +1 to		

				//ARRAYS TO COMPUTE THE SUPER-HISTOGRAM
	...
	//START THE VARIABLES OF HISTOGRAM**************************************************************************
	...
	for(l=1;l<=seed;l++)
	{
		...
		//START A TEMPORAL SERIES LOOP

				//INITIATION OF VARIABLES (IN EACH TEMPORAL SERIES IT MUST BE RESTARTED)
				...
				ciclo_step=2; //each many cycles it is measured
				ciclo_flag=6;  //cycle from which measures starts 
			...
			u1=SPmax*(gsl_rng_uniform (r));
				...
				else if(SP[0]<u1 && u1<=SP[1])
				{
					...
					{					
						...
						w=gsl_rng_uniform_int (r,Gtot)+1;
						Gtot=Gtot+1;
						ZG[Gtot]=1.0/(1.0/ZG[w]+gsl_ran_gaussian_tail(r, -(1.0/ZG[w]), (1.0/ZG[w])*mutaciones));
						ZG[w]=1.0/(1.0/ZG[w]+gsl_ran_gaussian_tail(r, -(1.0/ZG[w]), (1.0/ZG[w])*mutaciones));
						Ntot=Gtot+Ztot;
					}
					else
					{
						...
						ZG[w2]=1.0/(1.0/ZG[w]+gsl_ran_gaussian_tail(r, -(1.0/ZG[w]), (1.0/ZG[w])*mutaciones));
						ZG[w]=1.0/(1.0/ZG[w]+ggsl_ran_gaussian_tail(r, -(1.0/ZG[w]), (1.0/ZG[w])*mutaciones));
					}
				}
